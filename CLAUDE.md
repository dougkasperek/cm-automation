...
| **Consent gating sweep** | **built 2026-08-27, in CI 2026-08-28.** Clicks Reject All and reloads: do the tags actually stop? Its own source, `consent-gating`, because it asks a different question from the cold sweep and the consent MEASURED predicate is false on every gating row. First run: 26 tooled sites, 23 tested, **2 still firing** (MS Clarity, both Interstate Waste sites), 7 firing correctly as cookieless `gcs=G100` pings, 3 untested. Own page at `/consent`. **Scored since 2026-08-28** by `consent_gating_leak` on the consent axis. The blocker recorded here — "what counts as a pass is a question for Nick" — **was already answered when it was written**: `docs/CONSENT.md` has ruled since the first sweep that a cookieless `gcs=G100` ping after a rejection is correct behaviour, `check-site.mjs` and `test-gating.mjs` both implement it, and Nick asserted the same thing independently. A stale blocker held the codes for a day. Severity does not re-decide it: `gating_still_firing` already excludes the compliant Google case before the ledger sees it. **0 sites leak**, measured 2026-08-28. Untested is an info line, never a WARN — 9 of 27 could not be tested and a rule firing on all of them ranks nothing. **Both 2026-08-27 windows were wrong** (v1 counted the old page's tail, v2 erased the new page's load — see the bug table); the window is v3 since 2026-08-28 and the fleet numbers await a re-run |
...
| the banner's `a backlog (42 sites)` and `unmeasured (11 sites)`, above lane tiles reading 41 and 11 | **four numbers, two sources, and one pair agreed by coincidence.** `backlog` is `backlogOnly` (sites whose only warning is a backlog) against the `schedule` LANE, which excludes any site needing a person first — 42 against 41. `unmeasured` is `no_health_evidence` against the `unestablished` LANE: both were 11 on 2026-08-29 over DIFFERENT sets, the first holding `elderwoodipa.com` and not `app.eastauroracc.com`, the second the reverse. The banner sat directly above the tiles, so the page invited the comparison and then failed it. Found by Doug drawing a box round the two blocks and asking "can we combine these?" — the merge is the fix, and the clause came out of the red state; the green state keeps its equivalent because there it is the only thing stopping the banner reading as an all-clear |
| `a red banner names exactly the needs-a-person sites`, green for weeks | it never tested a name. It asserted `banner.text.includes(personN + ' site')`, and the HEADLINE already reads "4 sites need a person" — so replacing the entire site list with "See the matrix." passed. Found on 2026-08-29 by a negative control that failed to fail, which is the only way this kind is ever found: **a check whose title claims more than its predicate.** Now asserts every id in the lane appears in the banner prose, sourced from the matrix rows so the lane tile is cross-checked too; verified against both dropping all four names and dropping one |
| a headline block grouping the seven lanes into four cards, one of them `Not scored`, holding `4 Not measurable` and `1 Excluded by ruling` | **all five are scored, and one of them is the fleet's worst site.** `cm-whitelabel` is `production: false`, which means scored and NOT COUNTED — its own lane gloss says so — and on 2026-08-29 it was **CRIT with `wp_below_floor`, `backup_stale` and `wp_update_status_unknown`**. The other four are SKIP and FROZEN, which are severity statuses, not the absence of one. So a card reading "Fleet cannot produce a maintenance result" covered a site that had produced three findings. Wrong in the reassuring direction, which is the direction that matters. Caught in the concept, before it was built, by scoring the five rather than reading the label |
| the same block folding `11 Not established` in beside `41 Needs scheduling` and `17 Needs a ruling`, under "maintenance is not actionable yet because work must be scheduled, defined, or ruled on" | **on those 11 sites nothing is known, so there is no work to schedule and nothing to rule on.** `unestablished` is `health.status === "UNKNOWN"` or a code in `UNESTABLISHED` — on 2026-08-29, `coverage_partial` and `framework_not_wordpress`. A backlog is work you have measured; this lane is the absence of a measurement, and the group name asserted the opposite about it. **This file's cardinal rule, in the page's own headline block: unknown is a value, never folded into yes or no.** The absence gets its own card |

#!/usr/bin/env python3
# ---------------------------------------------------------------------------
# MOCKUP ONLY. A FORK OF scripts/render-dashboard.py, 2026-08-26.
#
# This is a THROWAWAY. It exists so the remaining batch-one UI changes can be
# reviewed as a full page and as a diff against the real renderer, rather than
# as a partial mockup that silently drops four sections and nine columns.
#
#   diff scripts/render-dashboard.py docs/mockups/render-batch-one.py
#
# It must end in one of two states: the changes are applied back to the real
# renderer and this file is DELETED, or the changes are rejected and this file
# is DELETED. A second copy of the thing that actually runs is a place for the
# two to disagree, and the copy that loses is always the one nobody is looking
# at -- see the gitignored ci/github-actions/ mirror in CLAUDE.md.
#
# Do not publish from this file. It writes to docs/mockups/ by default.
# ---------------------------------------------------------------------------
"""
...
# MOCKUP: this fork lives in docs/mockups/, so the ledger module is two
# directories up in scripts/. The real renderer sits beside it.
_spec = importlib.util.spec_from_file_location(
    "ledger", os.path.join(HERE, "..", "..", "scripts", "fleet-ledger.py"))
...
def render(m):
    ...
    md(A, "Where each record is looked up, and what is not verified", [
        ...
        "<code>example.com</code> through <code>web.example.com</code> has two "
        "answers and only one of them is what a recipient's mail server "
        "checks.",
        ...
        "The sending domain is a <b>ruling</b>. On 59 of 75 sites the From: "
        "address is now measured off the site itself, from post-smtp's own "
        "configuration, and stored beside the ruling rather than over it, so "
        "a disagreement shows instead of resolving silently. 37 of 38 "
